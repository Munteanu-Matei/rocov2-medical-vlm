#!/usr/bin/env python
# LoRA fine-tuning of Qwen3-VL-4B-Instruct on ROCOv2 WITH UMLS CONCEPT GUIDANCE.
#
# ============================================================================
# WHAT CHANGED vs qwen3vl_roco_finetune_cui.py  (Model B, first run)
# ============================================================================
# The first Model B run used a FIXED lambda:
#
#       L_total = L_caption + lambda * L_aux        with lambda = 0.1, constant
#
# lambda is constant, but the QUANTITY THAT MATTERS -- how much the auxiliary
# objective actually influences the shared visual representation -- is not
# lambda.  It is the ratio
#
#       influence = lambda * L_aux / L_caption
#
# and both losses move during training, at very different rates:
#
#       step    L_caption    L_aux    lambda   influence
#        750      2.380      0.826     0.1       3.5%
#       1500      2.240      0.455     0.1       2.0%
#       2250      2.215      0.406     0.1       1.8%
#       3000      2.176      0.361     0.1       1.7%
#       change     -9%        -56%      0%
#
# Predicting ~3 concepts from an image is easy, so the auxiliary head converged
# fast and L_aux collapsed by 56%.  Generating radiology prose is hard, so
# L_caption barely moved (-9%).  With lambda fixed, the auxiliary term therefore
# SHRANK relative to the caption term throughout training.  Since the gradient
# reaching the merger scales with lambda * dL_aux/dtheta, the concept signal was
# strongest while the model was still random and weakest once it mattered --
# and never entered the 5-20% band this script itself recommends.
#
# Model A and Model B came out statistically identical (val 2.2015 vs 2.1987,
# 0.13%).  That is NOT evidence that concept guidance fails: at a 1.7% influence
# the intervention was too weak to test the hypothesis at all.
#
# FIX: normalise lambda per step so the influence stays at a target fraction:
#
#       lambda_t = clamp(AUX_FRAC * L_caption.detach() / L_aux.detach(),
#                        LAMBDA_MIN, LAMBDA_MAX)
#
#   - .detach() is essential.  The scale factor must not be differentiated, or
#     the optimiser minimises the RATIO instead of the two losses.  Detached, it
#     acts as a per-step gradient rescale, which is what we want.
#   - the clamp guards the degenerate case L_aux -> 0  =>  lambda -> infinity.
#   - AUX_FRAC = 0.10 sits mid-range of the recommended 5-20%.  Higher values
#     trade caption quality for concept accuracy, and BERTScore is the metric
#     we report, so 0.20 is the ceiling worth trying.
#
# Set LAMBDA_MODE=fixed to recover the original constant-lambda behaviour
# (reproduces the first Model B run exactly when LAMBDA_CUI=0.1).
#
# ROCO_DIR is now read from the environment so the model can be trained on the
# caption-enriched labels:
#       ROCO_DIR=/home/matei/rocov2_enriched python qwen3vl_roco_finetune_cui_v2.py
# ============================================================================
#
...
#
#   probe : PROBE=1 python qwen3vl_roco_finetune_cui_v2.py
#   train : ROCO_DIR=/home/matei/rocov2_enriched python qwen3vl_roco_finetune_cui_v2.py \
#             2>&1 | tee ft_cui_v2_run.log
#   ablate: AUX_FRAC=0 python qwen3vl_roco_finetune_cui_v2.py   # == Model A, same code path
#   fixed : LAMBDA_MODE=fixed LAMBDA_CUI=0.1 python qwen3vl_roco_finetune_cui_v2.py
#   resume: RESUME=1 python qwen3vl_roco_finetune_cui_v2.py
import os
os.chdir("/home/matei")
...
# "adaptive": lambda_t is recomputed each step so that
#     lambda_t * L_aux / L_caption  ==  AUX_FRAC
# holding the auxiliary objective's influence CONSTANT as L_aux decays.
# "fixed"   : the original behaviour, lambda == LAMBDA_CUI for the whole run.
LAMBDA_MODE   = os.environ.get("LAMBDA_MODE", "adaptive")   # "adaptive" | "fixed"
AUX_FRAC      = float(os.environ.get("AUX_FRAC", "0.10"))   # target influence; 0 -> Model A
LAMBDA_CUI    = float(os.environ.get("LAMBDA_CUI", "0.1"))  # used only when MODE=fixed
LAMBDA_MIN    = float(os.environ.get("LAMBDA_MIN", "0.05")) # clamp: L_aux -> 0 => lambda -> inf
LAMBDA_MAX    = float(os.environ.get("LAMBDA_MAX", "5.0"))
ADAPTIVE      = (LAMBDA_MODE == "adaptive")
AUX_ON        = (AUX_FRAC > 0) if ADAPTIVE else (LAMBDA_CUI > 0)
...
if TRAIN_MERGER is False and AUX_ON:
    print("[warn] auxiliary loss enabled but TRAIN_MERGER=0: the auxiliary gradient has nowhere to go "
...
import pandas as pd
# env-readable so the CUI head can be supervised with the caption-enriched labels
ROCO_DIR = os.environ.get("ROCO_DIR", "/home/matei/rocov2")
print(f"[data] concept labels from {ROCO_DIR}", flush=True)
...
def forward_losses(rec, split_cui, split_man):
    """Returns (total, caption_loss, {aux parts}, lambda_used).

    aux is skipped when AUX_ON is False (AUX_FRAC=0, or LAMBDA_CUI=0 in fixed mode)."""
    ...
    l_cap = out.loss
    parts = {}
    if AUX_ON and _captured:
        pooled = pooled_visual(ex["image_grid_thw"])
        parts = aux_loss(pooled, [rec["id"]], split_cui, split_man)

    if not parts:
        return l_cap, l_cap, parts, 0.0

    aux = sum(parts.values())
    if ADAPTIVE:
        # Hold the auxiliary influence at AUX_FRAC of the caption loss.
        # BOTH operands are detached: this is a gradient RESCALE, not part of
        # the objective.  Differentiating it would make the optimiser minimise
        # the ratio (trivially, by inflating L_caption) instead of the losses.
        lam = (AUX_FRAC * l_cap.detach() / aux.detach().clamp_min(1e-8)
               ).clamp(LAMBDA_MIN, LAMBDA_MAX)
    else:
        lam = torch.as_tensor(LAMBDA_CUI, device=l_cap.device, dtype=l_cap.dtype)

    total = l_cap + lam * aux
    return total, l_cap, parts, float(lam)
...
def validate():
    ...
    for rec in tqdm(val_recs, desc="val", leave=False, mininterval=15):
        try:
            _, l_cap, _, _ = forward_losses(rec, cui_valid, man_valid)
            tot += l_cap.item(); n += 1
...
def save_ckpt(tag):
    ...
    if AUX_ON and head_params:              # not needed for inference; saved for analysis
...
if PROBE:
    ...
    for i in tqdm(range(PROBE_STEPS), desc="probe"):
        total, l_cap, parts, lam_used = forward_losses(train_recs[i], cui_train, man_train)
    ...
    print(f"  lambda mode    : {LAMBDA_MODE}  (target influence {AUX_FRAC:.0%})"
          if ADAPTIVE else f"  lambda         : {LAMBDA_CUI} (fixed)")
    print(f"  lambda applied : {lam_used:.3f}   loss: {CUI_LOSS}")
    ...
    print(f"  losses         : {last}")
    if aux_sum:
        print(f"  weighted aux / caption = {lam_used*aux_sum/last['caption']:.1%} "
              f"(adaptive mode holds this at AUX_FRAC={AUX_FRAC:.0%} for the whole run)",
              flush=True)
    raise SystemExit(0)
...
json.dump({"cui_vocab": CUI_VOCAB, "man_vocab": MAN_VOCAB, "lambda_mode": LAMBDA_MODE, "aux_frac": AUX_FRAC, "lambda": LAMBDA_CUI,
...
history, stop = [], False
run_cap, run_aux, run_lam, run_n = 0.0, 0.0, 0.0, 0
for epoch in range(math.ceil(EPOCHS)):
    ...
    for rec in pbar:
        try:
            total, l_cap, parts, lam_used = forward_losses(rec, cui_train, man_train)
            (total / GRAD_ACCUM).backward()
            run_cap += l_cap.item()
        ...
        if gstep % EVAL_EVERY == 0 or gstep == total_steps:
            ...
            ml = run_lam / max(1, run_n)                 # mean lambda actually applied
            infl = (ml * ma / mc) if mc else 0.0         # realised influence
            history.append({"step": gstep, "val_caption_loss": vl, "train_caption": mc,
                            "train_aux": ma, "lambda_mean": ml, "weighted_aux_frac": infl,
                            "lambda_mode": LAMBDA_MODE, "aux_frac_target": AUX_FRAC,
                            "hours": (time.time() - t0) / 3600})
            print(f"\n[eval] step {gstep} | val {vl:.4f} | best {best_val:.4f} | "
                  f"train cap {mc:.3f} aux {ma:.3f} | lambda {ml:.3f} | "
                  f"influence {infl:.1%} (target {AUX_FRAC:.0%})", flush=True)
            run_cap = run_aux = run_lam = 0.0; run_n = 0
            if vl < best_val:
