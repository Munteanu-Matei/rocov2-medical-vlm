#!/usr/bin/env python
"""Standalone BLIP-2 VQA-RAD fine-tuning — generated from blip-2_fine_tuned_VQA-RAD.ipynb.
Run inside tmux on the server:  python /home/matei/train.py
Saves checkpoints to /home/matei/vqa_checkpoints/ (every epoch + a final one)."""
import os
os.chdir("/home/matei")            # dataset paths in the notebook are relative to here


# ======================================================================
# cell: setup
# ======================================================================
# %pip install -U transformers accelerate datasets pillow tqdm
...
class Blip2QFormerVQA(nn.Module):
    ...
    def _qformer_features(self, pixel_values, q_ids, q_att):
        # 1. image features (ViT is frozen -> no_grad saves activation memory)
        with torch.no_grad():
...
# cell: instantiate
# ======================================================================
# ── Instantiate, cast the grafted embeddings, set what trains ────────────────
model = Blip2QFormerVQA(blip2, qformer_word_emb, qformer_pos_emb, qformer_text_ffn).to(device)
# match the grafted embeddings to the Q-Former's dtype/device (fp32 region in fp16 loads)
...
set_trainable(model.vision_model, False)     # ViT frozen (memory)
...
# gradient checkpointing on the frozen LLM cuts activation memory for the 12 GB card
...
EPOCHS       = 5      # paper (Table 8) uses 5
ACCUM_STEPS  = 4      # effective batch = 2 * 4 = 8 (fits 12 GB)
LR           = 1e-5   # paper's VQA fine-tuning LR
WEIGHT_DECAY = 0.05   # paper's value
SAVE_DIR     = "/home/matei/vqa_checkpoints"   # checkpoints written here (survive kernel death)
os.makedirs(SAVE_DIR, exist_ok=True)

def save_ckpt(tag):
    # Only the trainable parts are saved (~300 MB); the frozen ViT/LLM are reloaded from disk.
    ckpt = {
    ...
    p = os.path.join(SAVE_DIR, f"vqa_finetuned_{tag}.pt")
    torch.save(ckpt, p)
    return p

# Weight decay on matmul weights only. Biases, norms, embeddings and the query
# tokens are excluded (standard practice — decaying them hurts).
_no_decay_keys = ("bias", "norm", "word_emb", "pos_emb", "query_tokens")
_decay, _no_decay = [], []
for _n, _p in model.named_parameters():
    if not _p.requires_grad:
        continue
    if _p.ndim <= 1 or any(k in _n.lower() for k in _no_decay_keys):
        _no_decay.append(_p)
    else:
        _decay.append(_p)
optimizer = AdamW(
